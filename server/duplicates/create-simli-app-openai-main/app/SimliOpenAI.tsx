...
import { instructions, resumeTXT, JDTXT} from './utils/conversation_config';
...
const evaluationPrompt = `You are an expert interview evaluator. Given a job interview transcript, analyze the candidate's performance across key categories. For each category, provide a brief explanation and assign a numerical rating from 1 to 10 (1 = very poor, 10 = excellent). Conclude with a short overall summary.

Evaluate the following categories:
1. Strong Skills – What the candidate did well.
2. Weak Skills – Areas needing improvement.
3. Communication Style – Clarity, tone, professionalism, and empathy.
4. Industry Knowledge – Understanding of tools, trends, or domain-specific concepts.
5. Confidence Level – Poise, certainty, and self-assurance in responses.
6. Overall Impression – General readiness and suitability for the role.

Output Format:
Strong Skills: [your comments]  
Rating: X/10

Weak Skills: [your comments]  
Rating: X/10

Communication Style: [your comments]  
Rating: X/10

Industry Knowledge: [your comments]  
Rating: X/10

Confidence Level: [your comments]  
Rating: X/10

Overall Impression: [your comments]  
Rating: X/10
...
const SimliOpenAI: React.FC<SimliOpenAIProps> = ({
  ...
  const initializeSimliClient = useCallback(() => {
    if (videoRef.current && audioRef.current) {
      const SimliConfig = {
        apiKey: "8k8q7rtk5cc8g7cm3g0mnj", //process.env.NEXT_PUBLIC_SIMLI_API_KEY,
        faceID: simli_faceid,
        handleSilence: true,
  ...
  const initializeOpenAIClient = useCallback(async () => {
    try {
      ...
      openAIClientRef.current = new RealtimeClient({
        model: openai_model,
        apiKey: "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", //process.env.NEXT_PUBLIC_OPENAI_API_KEY,
      ...
      await openAIClientRef.current.connect().then(() => {
        ...
        openAIClientRef.current?.sendUserMessageContent([{
           type: `input_text`,
           text: `<Job_description>${JDTXT}</Job_description><IntervieweeResume>${resumeTXT}</IntervieweeResume>\n let start Interview:`}]);
  ...
  const handleEvaluation = useCallback(async () => {
    ...
    const ReportRes = await openAIClientRef.current.sendUserMessageContent([
      {
        type: "input_text",
        text: "Use the transcript history and generate an evaluation report based on the instructions.",
      },
    ]);
    console.log("Evaluation Report:", ReportRes);
